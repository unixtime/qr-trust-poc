 ...
 * nobody notices.
 *
 * This check scrapes the emitted literals straight out of the two services and
 * fails when one is not mapped, which turns "a new state renders amber" from a
...
const emittedDecisionStates = new Set([
  ...literals(verifier, /decision_state\s*=?=\s*"([a-z_]+)"/g),
  ...literals(verifier, /decision_state\s*=\s*"[a-z_]+"\s+if\s+[^\n]*?\s+else\s+"([a-z_]+)"/g),
])

check(
  emittedDecisionStates.size > 0,
  "extracted no decision states from verifier.py — the emitter changed shape",
)
for (const state of emittedDecisionStates) {
  check(
    decisionTones.has(state),
    `verifier.py emits decision_state "${state}" but DECISION_STATE_TONES does not map it`,
  )
}
  ...
  JSON.stringify(
    {
      ...
      emitted_decision_states_checked: emittedDecisionStates.size,
