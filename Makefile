...
help:
	...
	@printf "  make check-frontend-comparison Validate React A/B comparison layer mapping.\n"
	@printf "  make check-frontend-lifetime Validate the demo QR validity-window defaults.\n"
	...
	@printf "  make check-frontend-residuals Validate the residual lattice helpers.\n"
	@printf "  make check-frontend-scan-feedback Validate the scan feedback presentation.\n"
	@printf "  make check-python-verifier-lab-stability Run the fast Python lab stability gate.\n"
	@printf "  make capture-browser-evidence Capture accepted/mismatch browser screenshots.\n"
	...
	@printf "  make check-key-lifecycle-evidence Validate the public-safe E1/E2 key-lifecycle record.\n"
...
check-frontend-comparison:
	cd frontend && npm run comparison:smoke

check-frontend-lifetime:
	cd frontend && npm run lifetime:smoke
...
check-frontend-residuals:
	cd frontend && npm run residuals:smoke

check-frontend-scan-feedback:
	cd frontend && npm run scan-feedback:smoke
...
check-python-verifier-lab-stability:
	...
	@$(MAKE) check-frontend-comparison
	@$(MAKE) check-frontend-lifetime
	@$(MAKE) check-frontend-residuals
	@$(MAKE) check-frontend-scan-feedback
