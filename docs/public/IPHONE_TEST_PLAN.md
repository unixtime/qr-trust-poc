...
- Do not click the browser lab's `Check scanner decision` action before the
  phone scan. That can consume one-time QR state before the phone captures it.
- For one-time replay evidence, scan the same generated QR twice without
  regenerating it.
- For reusable-public evidence, repeated scans of the same QR should remain
  green until issuer, destination, runtime, or profile state changes.
...
| --- | --- | --- | --- |
| `green_reusable_public` | Scenario `valid`, Usage policy `reusable_public`, Nonce `timestamped` | Green `verified_issuer`; repeated scans stay valid | `accepted-reusable-public.png`, `history-accepted-reusable-public.png`, `accessibility-accepted-reusable-public.txt` |
| `green_one_time_first_pass` | Scenario `valid`, Usage policy `one_time`, Nonce `fixed`; first scan only | Green `accepted` | `accepted-one-time-first-pass.png`, `history-accepted-one-time-first-pass.png`, `accessibility-accepted-one-time-first-pass.txt` |
| `red_one_time_replay` | Reuse the exact QR from `green_one_time_first_pass`; scan it a second time | Red `one_time_replay` | `replay-guard.png`, `history-replay-guard.png`, `accessibility-replay-guard.txt` |
| `red_expired_qr` | Scenario `expired`, Usage policy `reusable_public`, Nonce `fixed` | Red `expired` | `expired.png`, `history-expired.png`, `accessibility-expired.txt` |
| `red_destination_mismatch` | Scenario `payload-mismatch`, Usage policy `reusable_public`, Nonce `timestamped` | Red `destination_policy_mismatch` | `payload-mismatch.png`, `history-payload-mismatch.png`, `accessibility-payload-mismatch.txt` |
| `red_resolver_final_target_mismatch` | Scenario `redirect-final-mismatch`, Usage policy `reusable_public`, Nonce `timestamped` | Red `resolver_final_target_mismatch` | `resolver-final-target-mismatch.png`, `history-resolver-final-target-mismatch.png`, `accessibility-resolver-final-target-mismatch.txt` |
...
- green reusable-public scans remain reusable
- one-time replay blocks only after the first accepted one-time scan
