...
| `red_resolver_final_target_mismatch` | Legacy basename: Scenario `redirect-final-mismatch`; current runtime must not trust its query-carried final target | Red `unknown`, binding `redirect_unobserved`, cause `resolution-unavailable`; no final URL or hop count asserted | `resolver-final-target-mismatch.png`, `history-resolver-final-target-mismatch.png`, `accessibility-resolver-final-target-mismatch.txt` |
...
status: Red status
title: Do not open
message: The encoded URL changed or is outside the issuer-approved policy.
