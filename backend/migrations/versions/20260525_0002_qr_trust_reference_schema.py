"""Add QR Trust reference schema tables.
    ...
    _execute_sql_script(
        ...
        create table if not exists qr_trust.delegated_authorities (
          root_program_id text not null
            references qr_trust.root_programs(root_program_id),
        ...
        create table if not exists qr_trust.trust_keys (
          ...
          foreign key (root_program_id, delegated_authority_id)
            references qr_trust.delegated_authorities(
              root_program_id,
        ...
        create table if not exists qr_trust.issuer_certificates (
          ...
          key_status text not null default 'active'
            check (
              key_status in ('active', 'rotated', 'suspended', 'revoked', 'expired')
            ),
        ...
        create table if not exists qr_trust.published_artifacts (
          ...
          publication_status text not null default 'published'
            check (
              publication_status in ('draft', 'published', 'superseded', 'revoked')
            ),
        ...
        create index if not exists verifier_cache_entries_expiry_idx
          on qr_trust.verifier_cache_entries (
            verifier_id,
            freshness_status,
            cache_expires_at
          );
        ...
        create table if not exists qr_trust.runtime_observations (
          ...
          verdict text not null
            check (
              verdict in ('clear', 'risky', 'blocked', 'unknown', 'unavailable')
            ),
...
def downgrade() -> None:
    _execute_sql_script(
        """
        drop table if exists qr_trust.event_outbox;
        drop table if exists qr_trust.scanner_decisions;
        drop table if exists qr_trust.runtime_observations;
        drop table if exists qr_trust.verifier_cache_entries;
        drop table if exists qr_trust.verifier_cache_work_items;
        drop table if exists qr_trust.status_events;
        drop table if exists qr_trust.artifact_publication_work_items;
        drop table if exists qr_trust.published_artifacts;
        drop table if exists qr_trust.destination_policies;
        drop table if exists qr_trust.issuer_certificates;
        drop table if exists qr_trust.issuer_domain_proofs;
        drop table if exists qr_trust.issuers;
        drop table if exists qr_trust.trust_keys;
        drop table if exists qr_trust.delegated_authorities;
        drop table if exists qr_trust.root_programs;
        """
    )
